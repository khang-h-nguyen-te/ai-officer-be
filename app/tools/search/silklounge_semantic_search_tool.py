...
class SilkLoungeSemanticSearchTool(BaseTool):
    """Tool for semantically searching information using Supabase vector search."""
    
    def __init__(self):
        super().__init__(
            name="SilkLoungeSemanticSearch",
            description="Search for information about Silk Lounge using semantic search. This tool helps retrieve relevant information to answer user queries about Silk Lounge services, amenities, and FAQs."
        )
    ...
    def __call__(self, query: str, match_count: int = 3) -> str:
        ...
        try:
            ...
            logger.info(f"Calling semantic_search_silklounge with match_count={match_count}")
            response = self.supabase.rpc(
                'semantic_search_silklounge',  # Note: You'll need to create this function in Supabase
                {
